...
#link("https://en.wikipedia.org/wiki/Project_management", [project management])!
This has given them the tools they need to prevent their usual Christmas
emergency. For example, they now know that the North Pole decorations need to be
...
`0` through `99` in order. As you turn the dial, it makes a small *click* noise
...
or `R` which indicates whether the rotation should be to the `left` (toward
lower numbers) or to the `right` (toward higher numbers). Then, the rotation has
a `distance` value which indicates how many clicks the dial should be rotated in
that direction.
...
Your puzzle answer was #raw(str(answers.at(0))).
...
Your puzzle answer was #raw(str(answers.at(1))).
